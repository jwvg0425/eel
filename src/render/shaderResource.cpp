...
eel::ShaderResource::ShaderResource(SPTR<Texture> tex)
{
	if (tex->GetType() == TextureType::NORMAL)
	{
		HR(Renderer::GetInstance()->GetDevice()->CreateShaderResourceView(
			tex->GetTexture(), 0, &m_Resource));
	}
	else
	{
		...
		Renderer::GetInstance()->GetDevice()->CreateShaderResourceView(tex->GetTexture(), &srvDesc, &m_Resource);
	}
}
